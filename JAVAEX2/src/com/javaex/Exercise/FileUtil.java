	...
	public static void writeFile(String dir, String name, List<PersonInfo> InfoList ) throws IOException {
        ...
        try {
            ...
            for(int idx=0; idx < InfoList.size(); idx++) {
                //List에서 개인정보를 가져온다.
                String writeStr = InfoList.get(idx).getId() +"," + InfoList.get(idx).getName() + "," + 
                					InfoList.get(idx).getPnum() + ","
                                + InfoList.get(idx).getHnum() +"\n";
    ...
    public static List<PersonInfo> readReader(Reader input) throws IOException {
        try {
            ...
            while((line=in.readLine()) != null) {
                ...
                //각 개인정보를 저장한다.
                personInfo.setId(Integer.parseInt(writeStr[0]));
                personInfo.setName(writeStr[1]);
                personInfo.setPnum(writeStr[2]);
                personInfo.setHnum(writeStr[3]);
